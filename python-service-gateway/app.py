...
metrer = metrics.get_meter(__name__)

def memory_usage_callback(options):
    print("observing gauges")
    memory_usage = psutil.Process().memory_info().rss
    yield metrics.Observation(memory_usage)

memory_usage_callback = metrer.create_observable_gauge(
...
api_duration = metrer.create_gauge("gateway_api_duration", "Duration of the API request", "ms")
